...
// gRPC-Web test functions for projects
func testListProjectsGRPCWeb(t *testing.T) {
	...
	t.Logf("Successfully listed projects via gRPC-Web")
	t.Logf("Response snippet: %.500s...", text)
}

func testGetProjectGRPCWeb(t *testing.T) {
	// Try to get the default project which should always exist
	projectName := "default"
	...
	t.Logf("Successfully retrieved project %s via gRPC-Web", projectName)
	t.Logf("Response snippet: %.500s...", text)
}

func testInvalidProjectNameGRPCWeb(t *testing.T) {
	mcpCmd, stdin, stdout := startMCPServerWithGRPCWeb(t)
	defer func() {
		_ = mcpCmd.Process.Kill()
		_ = mcpCmd.Wait()
	...
	if errObj, ok := response["error"].(map[string]interface{}); ok {
		t.Logf("Received expected error for non-existent project via gRPC-Web: %v", errObj["message"])
	} else if result, ok := response["result"].(map[string]interface{}); ok {
		content, _ := result["content"].([]interface{})
		if len(content) > 0 {
			textContent, _ := content[0].(map[string]interface{})
			text, _ := textContent["text"].(string)
			if strings.Contains(strings.ToLower(text), "not found") || strings.Contains(strings.ToLower(text), "error") {
				t.Log("Received expected error message for non-existent project via gRPC-Web")
			} else {
				t.Errorf("Expected error for non-existent project, but got: %s", text)
			}
		}
...
// TestCreateProjectGRPCWeb tests the create_project tool via gRPC-Web
func testCreateProjectGRPCWeb(t *testing.T) {
	// Use a unique project name to avoid conflicts
	projectName := fmt.Sprintf("test-project-grpcweb-%d", time.Now().Unix())
	...
	callToolRequest := map[string]interface{}{
		...
		"params": map[string]interface{}{
			...
			"arguments": map[string]interface{}{
				"name":                  projectName,
				"description":           "Test project created via gRPC-Web",
				"source_repos":          "*",
				"destination_server":    "https://kubernetes.default.svc",
				"destination_namespace": "*",
				"upsert":                true, // Use upsert to avoid conflicts in repeated tests
			},
		},
	}
	...
	if errObj, ok := response["error"].(map[string]interface{}); ok {
		...
		t.Fatalf("Error calling create_project via gRPC-Web: %v", errorMsg)
	}
	...
	t.Logf("Successfully created project %s via gRPC-Web", projectName)
	t.Logf("Response snippet: %.500s...", text)
}
